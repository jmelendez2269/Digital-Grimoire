...
test("wallet loader scopes the service projection to the authenticated user", async () => {
  ...
  assert.deepEqual(calls, [[USER_ID, "2026-08-11T12:01:00.000Z", 20]]);
...
test("invalid identity and time stop before privileged loading", async () => {
  ...
  await assert.rejects(
    getCreditWalletForUser("not-a-user", {
      loadWallet,
      resolveEntitlement: async () => paidEntitlement(),
    }),
...
test("malformed, inconsistent, and overlong projections fail closed", async () => {
  for (const value of [
    ...
    projection({
      history: Array.from({ length: 21 }, () => projection().history[0]),
    }),
    projection({ grant: { ...projection().grant, planCode: "premium" } }),
    projection({
      pending: [{ ...projection().pending[0], actionCode: "BAD ACTION" }],
    }),
  ]) {
    await assert.rejects(
      getCreditWalletForUser(USER_ID, {
...
test("Reader remains ineligible even while a legacy balance is being retired", async () => {
  const wallet = await getCreditWalletForUser(USER_ID, {
    loadWallet: async () => projection(),
  ...
  assert.equal(wallet.planCode, "reader");
  assert.equal(wallet.paidCreditsActive, false);
  assert.equal(wallet.availableCredits, 8);
});
...
test("the API derives scope only from auth and the database function is service-only", () => {
  ...
  const paidOnlyMigration = readFileSync(
    resolve(
      appRoot,
      "../supabase/migrations/20260813000000_paid_only_membership_credits.sql"
    ),
    "utf8"
  );
  ...
  assert.match(
    migration,
    /revoke all on function public\.get_credit_wallet_v1/
  );
  ...
  assert.match(paidOnlyMigration, /return 'reader_no_credit'/);
  assert.match(
    paidOnlyMigration,
    /return public\.sync_monthly_credit_grant_legacy_v1/
  );
  assert.match(paidOnlyMigration, /'PAID_ONLY_CREDITS'/);
  assert.doesNotMatch(paidOnlyMigration, /v_target_amount\s*:=\s*10/);
  assert.doesNotMatch(
    paidOnlyMigration,
    /grant execute on function public\.sync_monthly_credit_grant_legacy_v1/
  );
});
