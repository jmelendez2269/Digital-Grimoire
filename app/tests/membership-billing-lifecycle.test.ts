...
} from "../src/lib/membership/membership-billing-presentation";
      ...
      data: [
        {
          ...
          price: {
            id: overrides.priceId ?? "price_student15",
            object: "price",
          },
        },
      ],
...
test("founding renewal, cancellation scheduling, and reactivation retain exact $15 cohort", () => {
  ...
  for (const value of cases) {
    const normalized = normalizeMembershipSubscriptionSnapshot(
      value,
      environment
    );
...
test("account billing presentation accepts only the safe server projection", () => {
  const billing = billingSummaryFromProjection(
    projection(),
    environment,
    new Date("2026-08-15T00:00:00Z")
  );
  assert.deepEqual(parseSafeBillingSummary(billing), billing);
  assert.equal(
    parseSafeBillingSummary({ ...billing, amountCents: 1500.5 }),
    null
  );
  assert.equal(
    parseSafeBillingSummary({ ...billing, stripeCustomerId: "cus_forbidden" }),
    null
  );
  assert.equal(
    parseSafeBillingSummary({ ...billing, portalAvailable: "true" }),
    null
  );
});

test("account billing timeline distinguishes renewal, scheduled cancellation, and ended access", () => {
  const active = billingSummaryFromProjection(
    projection(),
    environment,
    new Date("2026-08-15T00:00:00Z")
  );
  assert.deepEqual(billingTimelineFromSummary(active), {
    kind: "renewal",
    date: "2026-09-01T00:00:00.000Z",
  });
  assert.deepEqual(
    billingTimelineFromSummary({ ...active, cancelAtPeriodEnd: true }),
    {
      kind: "scheduled_end",
      date: "2026-09-01T00:00:00.000Z",
    }
  ...
  assert.equal(
    billingTimelineFromSummary(billingSummaryFromProjection(null, environment)),
    null
  );
});
...
test("routes and migration keep Checkout return, Stripe IDs, and Journal work contained", () => {
  ...
  const billing = readSource("src/lib/membership/membership-billing.server.ts");
  ...
  assert.doesNotMatch(
    summary,
    /stripe_customer_id[\s\S]*jsonResponse\(\{\s*stripe/
  );
...
test("account billing UI renders server truth without legacy sync or duplicate subscription paths", () => {
  const account = readSource("src/components/SubscriptionTab.tsx");

  assert.match(account, /fetch\("\/api\/membership\/billing-summary"/);
  assert.match(account, /cache: "no-store"/);
  assert.match(account, /billing\.planName/);
  assert.match(account, /billing\.amountCents/);
  assert.match(account, /billing\.pricingCohort/);
  assert.match(account, /billing\.stripeStatus/);
  assert.match(account, /billing\.cancelAtPeriodEnd/);
  assert.match(account, /billing\.portalAvailable/);
  assert.match(account, /Founding rate/);
  assert.match(account, /while this subscription remains\s+uninterrupted/);
  assert.match(account, /payment method, invoices,\s+or cancellation/);
  assert.match(account, /fetch\("\/api\/stripe\/create-portal-session"/);

  assert.doesNotMatch(account, /\.from\(["']users["']\)/);
  assert.doesNotMatch(account, /sync-subscription|sessionStorage/);
  assert.doesNotMatch(account, /parallax\/rate-limit|queryLimit|queryCount/);
  assert.doesNotMatch(
    account,
    /NEXT_PUBLIC_STRIPE|stripe_customer_id|stripe_subscription_id/
  );
  assert.doesNotMatch(
    account,
    /create-checkout-session|handleUpgrade|offerCode/
  );
});
