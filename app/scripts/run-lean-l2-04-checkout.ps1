...
$checkoutMigrationPath = Join-Path $repositoryDirectory "supabase\migrations\20260811210000_lean_l2_04_checkout_requests.sql"
$testPath = Join-Path $appDirectory "tests\sql\lean-l2-04-checkout-requests.sql"

foreach ($requiredPath in @($membershipMigrationPath, $checkoutMigrationPath, $testPath)) {
  if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
    throw "Required LEAN-L2-04 SQL file was not found: $requiredPath"
  }
}
...
Write-Host "LEAN-L2-04 local dependency and forward migration"
Invoke-LocalSql -Path $membershipMigrationPath | Out-Null
Invoke-LocalSql -Path $checkoutMigrationPath | Out-Null

Write-Host "LEAN-L2-04 local schema, authorization, and replay verification"
$runId = [guid]::NewGuid().ToString()
$result = Invoke-LocalSql -Path $testPath -AdditionalVariables @(
  "--set=run_id=$runId"
)

if ($result -notmatch "local\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*PASS") {
  throw "LEAN-L2-04 PASS summary was not present."
}
if ($result -notmatch "cleanup_residue\s*(?:\r?\n)[-+]+(?:\r?\n)\s*0") {
  throw "LEAN-L2-04 did not prove zero fixture residue."
}

Write-Host "LEAN_L2_04_LOCAL_RESULT: PASS"
