...
$migrationPath = Join-Path $repositoryDirectory "supabase\migrations\20260810220000_lean_l1_02_learner_progress.sql"
$testPath = Join-Path $appDirectory "tests\sql\lean-l1-02-learner-progress.sql"

foreach ($requiredPath in @($migrationPath, $testPath)) {
  if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
    throw "Required LEAN-L1-02 SQL file was not found: $requiredPath"
  }
}
...
Write-Host "LEAN-L1-02 local forward migration"
Invoke-LocalSql -Path $migrationPath | Out-Null

Write-Host "LEAN-L1-02 local authorization, revision, and replay verification"
$runId = [guid]::NewGuid().ToString()
$result = Invoke-LocalSql -Path $testPath -AdditionalVariables @(
  "--set=run_id=$runId"
)

if ($result -notmatch "local\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*1\s*\|\s*PASS") {
  throw "LEAN-L1-02 PASS summary was not present."
}
if ($result -notmatch "cleanup_residue\s*(?:\r?\n)[-+]+(?:\r?\n)\s*0") {
  throw "LEAN-L1-02 did not prove zero fixture residue."
}

Write-Host "LEAN_L1_02_LOCAL_RESULT: PASS"
