...
export default function AdminCoursesPage() {
  ...
  const moveOrder = async (index: number, direction: "up" | "down") => {
    ...
    const changedCourses = normalized.filter((course, orderIndex) => {
      const previousCourse = previousCourses.find(
  ...
  const handleTogglePublish = async (
    ...
    setCourses((prev) =>
      prev.map((course) =>
        course.id === courseId
          ? { ...course, is_published: nextPublished }
          : course
      )
    );
    ...
    try {
      ...
      if (!data.success) {
        ...
        setError(
          data.error ||
            `Failed to ${nextPublished ? "publish" : "unpublish"} course`
        );
      }
    } catch (error) {
      ...
      <main className="container mx-auto mt-24 flex-grow px-4 py-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-zinc-400">
              Create, reorder, edit, publish, and delete courses.
            </p>
          </div>
        ...
        <div className="relative mb-6 max-w-md">
          ...
          <input
            ...
            className="w-full rounded-lg border border-zinc-800 bg-zinc-900/50 py-2 pr-4 pl-10 text-sm transition-all focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 focus:outline-none"
          />
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-dashed border-zinc-800 bg-zinc-900/20 py-20 text-center">
            ...
            <p className="mt-1 text-zinc-500">
              Create one here, or import one from the uploader.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {!search && (
              <p className="mb-4 text-xs text-zinc-500">
                This order is used for course display. Use the arrows to reorder
                the catalog.
              </p>
            )}
              ...
              >
                  ...
                  <>
                    <div className="flex shrink-0 flex-col gap-0.5">
                      ...
                      <button
                        ...
                        disabled={
                          index === courses.length - 1 || reordering !== null
                        }
                        className="p-0.5 text-zinc-600 transition-colors hover:text-zinc-300 disabled:cursor-not-allowed disabled:opacity-20"
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-center gap-3">
                    ...
                    <span className="shrink-0 text-xs tracking-wider text-zinc-500 uppercase">
                      {course.level}
                    </span>
                ...
                <div className="flex shrink-0 items-center gap-4">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      void handleTogglePublish(course.id, !course.is_published)
                    }
