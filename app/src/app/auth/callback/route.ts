...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    if (data.session && data.user) {
      ...
      const rawUserMetadata = asRecord(userRecord.raw_user_meta_data);
      const rawAppMetadata = asRecord(userRecord.raw_app_meta_data);
      const currentAvatarUrl = getString(userMetadata, "avatar_url");
      ...
      const googleAvatarUrl =
        ...
        getString(rawUserMetadata, "avatar_url") ||
        getString(rawUserMetadata, "picture") ||
        getString(rawAppMetadata, "avatar_url") ||
        getString(rawAppMetadata, "picture");

      const hasNoAvatar = !currentAvatarUrl || currentAvatarUrl.trim() === "";
      const avatarNeedsUpdate =
        googleAvatarUrl &&
        (hasNoAvatar || currentAvatarUrl !== googleAvatarUrl);
      ...
      if (avatarNeedsUpdate) {
        try {
          ...
          if (updateError) {
            console.error(
              "Failed to sync Google profile picture:",
              updateError
            );
          } else {
      ...
      if (user.email) {
        try {
          ...
          if (existingAdminProfile) {
            ...
            if (bulkUpdateError) {
              console.error(
                "Failed to update all profiles to admin:",
                bulkUpdateError
              );
            } else {
              console.log(
                "Ensured all profiles with this email have admin role"
              );
            }
          }
        } catch (err) {
