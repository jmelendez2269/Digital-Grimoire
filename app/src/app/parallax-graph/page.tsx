...
// Dynamically import ParallaxGraph to avoid SSR issues with D3
const ParallaxGraph = dynamic(
...
export default function ParallaxGraphPage() {
  const [concepts, setConcepts] = useState<ParallaxConcept[]>([]);
  const [relationships, setRelationships] = useState<ParallaxRelationship[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedConcept, setSelectedConcept] = useState<ParallaxConcept | null>(null);
    ...
    const loadData = async () => {
      try {
        ...
        if (!conceptsRes.ok || !relationshipsRes.ok) {
          throw new Error("Failed to load parallax data");
        }
        ...
        setError(e?.message || "Failed to load parallax graph data");
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="max-w-screen-2xl mx-auto px-4 py-8">
            ...
            {loading ? (
              <div className="flex items-center justify-center py-20">
                <div className="text-center">
                  <div className="w-16 h-16 border-4 border-amber-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
                  <p className="text-amber-100/60">Loading parallax data…</p>
                </div>
              </div>
            ) : error ? (
