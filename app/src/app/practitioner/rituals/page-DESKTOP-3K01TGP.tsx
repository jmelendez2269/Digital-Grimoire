...
export default async function RitualsPage() {
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            <DocumentationLink href="/wiki/user/workbench" label="Workbench Guide" />
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-3xl font-serif text-amber-500 mb-2">Ritual Workbench</h1>
                    <p className="text-zinc-400">Design and perform your personal rites.</p>
                </div>
                <Link
                    href="/practitioner/rituals/create"
            ...
            {(!rituals || rituals.length === 0) ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {rituals.map((ritual: any) => (
                        <Link
                            key={ritual.id}
                            href={`/practitioner/rituals/${ritual.id}/active`}
