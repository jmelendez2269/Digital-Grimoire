 ...
 * Verifies API token is present and valid
 */
export async function checkReplicateStatus(): Promise<{
  ...
  try {
    ...
    // Simple validity check using a lightweight endpoint (e.g. account or just model get)
    // Using Replicate client to getting a model info is a good liveness check
    const replicate = new Replicate({
      auth: apiKey,
    });

    // Check if we can fetch the model we actually use
    try {
      // Intentionally just checking the object instantiation for now as a "configured" check
      // To strictly check availability we'd need to make a network call
      // "replicate.hardware.list()" or getting a model.
      // fetch to api.replicate.com/v1/account is not exposed in the SDK directly easily without making a helper

      // Let's do a direct fetch to check the token which is cheaper/faster than loading a model metadata
      ...
      if (!response.ok) {
        if (response.status === 401) {
          return { configured: true, available: false, error: 'Invalid API Token' };
        }
        // If 404/other it might be a scope thing, but let's assume if it fails it's "Unavailable"
        return { configured: true, available: false, error: `API Error: ${response.status}` };
      }
      ...
      return {
        configured: true,
        available: true,
        model: 'flux-schnell', // Hardcoded as primary model
      };
