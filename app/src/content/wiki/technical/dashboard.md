...
3. **Tools Grid** (Primary Navigation):
    ...
    * **Seven Lenses** (`<ParallaxEngineInfo />`): Link to the 7-lens AI reasoning surface at `/seven-lenses` (the underlying component name still uses the legacy `ParallaxEngine` prefix per the rename matrix).
