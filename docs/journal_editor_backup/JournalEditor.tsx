...
export default function JournalEditor({
  ...
  const debouncedUpdateRef = useRef<(content: string) => void>();
  ...
  const editor = useEditor({
    ...
    // Don't pass content here - it causes re-initialization
    editorProps: {
      attributes: {
        class:
    ...
    onUpdate: ({ editor }) => {
      ...
      if (autoSave && debouncedUpdateRef.current) {
        setSaveStatus('unsaved');
        debouncedUpdateRef.current(json);
      } else {
        onUpdate(json);
      }
    },
  ...
  // Initial content load - only once when editor is created
  useEffect(() => {
    if (editor && !initialLoadRef.current && content) {
  ...
  // Update editor when content prop changes (e.g., switching pages)
  // Only update if we're not in the middle of editing
  useEffect(() => {
    if (!editor || !initialLoadRef.current) return;
    
    // Don't update if we're currently saving or editing
